...
<section class="header">
    ...
    <div class="chips">
        ...
        <span class="chip">{{ ($blockedUsers ?? collect())->count() }} Diblokir</span>
    </div>
</section>
            ...
            @foreach ($searchResults as $result)
                <div class="friend-card">
                    ...
                    <div class="actions">
                        ...
                        @elseif (in_array($result['status'], ['pending', 'accepted'], true))
...
<section class="layout">
            ...
            @forelse ($friends as $friend)
                ...
                <div class="friend-card">
                    ...
                    <div class="actions">
                        ...
                        <form action="{{ route('dashboard.friends.remove', $friend['id']) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-red-soft">❌ Hapus</button>
                        </form>
                        <form action="{{ route('dashboard.friends.block', $friend['id']) }}" method="POST">
                            @csrf
    ...
    <article class="section-card panel">
        ...
        <div class="list">
            @forelse (($blockedUsers ?? collect()) as $blocked)
                <div class="friend-card" style="padding:.65rem;">
                    <div class="info">
                        <div class="avatar">{{ strtoupper(substr($blocked['name'] ?? 'U', 0, 1)) }}</div>
                        <div>
                            <div class="name">{{ $blocked['name'] }}</div>
                            <div class="meta">@{{ $blocked['username'] }} • {{ $blocked['email'] }}</div>
                        </div>
                    </div>
                    <div class="actions">
                        <span class="btn" style="border-color:#fecdd3;background:#fff1f2;color:#9f1239;cursor:default;">BLOCKED</span>
